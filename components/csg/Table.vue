...
const isParamChanged = computed(() => {
	...
	if (props.params.sortType !== null && props.params.sortType !== '') return true
	return false
})
...
const sortedByLabel = computed(() => {
	const matchedColumn = props.columns.find((column) => column.key === props.params.sortBy)
		...
		if (localParams.search && localParams.searchBy.length > 0) {
			rows = rows.filter((row) => {
				return localParams.searchBy.some((key) => {
					return row[key] && row[key].toString().toLowerCase().includes(localParams.search.toLowerCase())
				})
			})
		}
...
const computedRows = computed(() => {
	if (props.type === 'client') {
		const rows = filteredRows.value.slice((localParams.currentPage - 1) * localParams.perPage, localParams.currentPage * localParams.perPage)
		return rows
	}
	else {
...
<template>
	<div class="csg-table">
		...
		<div class="csg-table__table">
			<div
				v-if="isShowingHeader"
				class="csg-table__header"
			>
			...
			<div
				class="csg-table__content"
				:class="{ 'csg-table__content--no-header': !isShowingHeader }"
			>
				<table class="content__table">
					<thead class="table__head">
						<tr>
							...
							<th
								...
								:class="{ 'cursor-pointer': column.isSortable === undefined || column.isSortable }"
							...
							>
								<div
									:class="[
										...
										{ 'head__wrapper--sortable': column.isSortable === undefined || column.isSortable }
									]"
								>
									<span v-if="column.isHeaderHtml">
										<slot :name="`header-column-${column.key}`" />
									</span>
									<span v-else-if="!column.isHeaderHtml">{{ column.label }}</span>
									<span
										v-if="hoveredHeader === column.key && (column.isSortable === undefined || column.isSortable)"
										class="head__sort"
									>
										<VueFeather
											:class="
									...
									<span
										v-else-if="
											localParams.sortBy === column.key && (column.isSortable === undefined || column.isSortable)
										"
										class="head__sort"
									>
										<VueFeather
											class="text-csg-tertiary"
											:type="localParams.sortType === 'ascending' ? 'chevron-up' : 'chevron-down'"
											size="0.75rem"
										/>
									</span>
								</div>
					...
					<tbody
						v-if="isFetching || !computedRows || (!isFetching && !computedRows.length)"
						class="table__body"
					>
						<tr
							v-if="isFetching"
					...
					>
						...
						>
							...
							>
								<span>
									{{ (localParams.currentPage - 1) * localParams.perPage + rowIndex + 1 }}
								</span>
							</td>
							<td
								v-for="(column, columnIndex) in columns"
